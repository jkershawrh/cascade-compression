...
**Status: LIVE in production.** All three systems deployed and running autonomously. 500K+ entries, 10% LLM-validated disagreement rate.
...
- Deploy to the production cluster (operational, not code)
...
6. **Deploy the ledger to the production cluster**: Operational, no code changes to ledger
7. **Deploy GCL to the production cluster**: With cascade audit config enabled
