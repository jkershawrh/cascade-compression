...
Red Hat OpenShift (Kubernetes) — 142 million signals from six clusters:
    ...
    Compression:         99.2%
    LLM classifications: 5,685 out of 142,000,000
    Activated agents:    3 (self-discovered)
    Shadow demotions:    0

The LLM classified 0.004% of signals. The rest were handled by deterministic agents in microseconds.

Red Hat Ansible Automation Platform — 2.5 million job signals from production:

    Compression:         98.5%
    Shadow checks:       457
    Shadow demotions:    32 (agents caught and deactivated)
    Self-correction:     real-time, no human intervention

Shadow validation caught 32 agents making mistakes and deactivated them instantly. The framework corrected itself while running.
